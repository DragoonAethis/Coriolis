...
class CrewIndexNewView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'event': self.event,
            'find_form': CrewFindTicketForm(event=self.event)
        })
...
class CrewFindTicketView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:  # Maybe it's a code?
            ...
            messages.warning(self.request, _("Ticket with the provided code does not exist - searching..."))
        ...
        try:
            ...
            if len(tickets) == 0:
                ...
                return render(self.request, 'events/crew/list.html', {
                    'event': self.event,
                    'form': form,
                    'tickets': tickets
                })
...
class CrewExistingTicketView(FormView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        elif self.ticket.status not in (TicketStatus.READY_PAY_ON_SITE, TicketStatus.READY_PAID):
    ...
    def get_context_data(self, **kwargs):
        ...
        other_user_tickets = list(Ticket.objects.filter(user_id=self.ticket.user.id)
